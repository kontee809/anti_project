...
const MarkerPopup = ({ data }) => {
  ...
  const getIcon = () => {
    // simplified icon text for header
  ...
  return (
    <div className="w-64 bg-white rounded-xl shadow-2xl overflow-hidden border border-slate-100">
      <div className={`${getHeaderStyle()} text-white px-4 py-3 font-semibold flex items-center justify-between`}>
        <span className="truncate pr-2" title={data.name}>{data.name}</span>
        <span className="text-lg opacity-80">{getIcon()}</span>
      </div>
      <div className="p-4 space-y-4">
        {data.type !== 'warning' ? (
          ...
          <div className="text-sm font-medium text-rose-600 border-l-2 border-rose-500 pl-3 bg-rose-50 p-2 rounded-r-md">
            {data.message}
          </div>
        )}
