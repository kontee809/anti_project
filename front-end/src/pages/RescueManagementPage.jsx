  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full page-bg min-h-full" data-test-id="rescue-mgmt-page">
          ...
          <table className="ui-table" data-test-id="rescue-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={req.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors" data-test-id={`rescue-row-${req.id}`}>
      ...
      {selectedReq && !showAssignDialog && (
        <div className="fixed inset-0 z-[2000] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4" data-test-id="rescue-detail-modal">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h2 className="text-xl font-bold text-slate-800">Chi tiết cứu trợ #{selectedReq.id}</h2>
              <button onClick={() => setSelectedReq(null)} className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full shadow-sm" data-test-id="rescue-modal-btn-close"><X size={20}/></button>
            </div>
      ...
      {showAssignDialog && selectedReq && (
        <div className="fixed inset-0 z-[3000] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4" data-test-id="rescue-assign-modal">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <input type="text" value={assignTeamName} onChange={(e) => setAssignTeamName(e.target.value)} placeholder="Tên đội / Đơn vị..." className="w-full p-3 rounded-xl border border-slate-200 mb-6 font-medium outline-none focus:ring-2 focus:ring-orange-500" autoFocus data-test-id="rescue-assign-input-team" />
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowAssignDialog(false)} className="px-4 py-2 text-slate-500 font-semibold hover:bg-slate-100 rounded-xl transition-colors" data-test-id="rescue-assign-btn-cancel">Hủy</button>
              <button onClick={() => handleAction(selectedReq.id, 'assign', { assignedTo: assignTeamName })} disabled={!assignTeamName.trim()} className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition-colors disabled:opacity-50" data-test-id="rescue-assign-btn-confirm">Xác nhận</button>
            </div>
          </div>
        </div>
      )}
