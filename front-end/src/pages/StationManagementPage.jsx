  ...
  return (
    <div className="p-8 max-w-7xl mx-auto w-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => handleOpenModal()} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700 transition">
            ...
            {loading ? <tr><td colSpan="7" className="p-8 text-center text-slate-500">Đang tải...</td></tr> : stations.map(s => (
              <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="p-4 flex gap-2 justify-end">
                  <Link to={`/stations/${s.id}`} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg transition" title="Xem biểu đồ"><Activity size={18}/></Link>
                  <button onClick={() => handleOpenModal(s)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition" title="Sửa"><Edit size={18}/></button>
                  <button onClick={() => handleDelete(s.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition" title="Xóa"><Trash2 size={18}/></button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 z-[3000] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className="text-sm font-semibold text-slate-700">Tên trạm</label><input required className="w-full mt-1 p-3 border rounded-xl font-medium" value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} /></div>
              <div><label className="text-sm font-semibold text-slate-700">Địa chỉ</label><input required className="w-full mt-1 p-3 border rounded-xl font-medium" value={formData.location} onChange={e=>setFormData({...formData, location: e.target.value})} /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-sm font-semibold text-slate-700">Vĩ độ (Lat)</label><input type="number" step="any" required className="w-full mt-1 p-3 border rounded-xl font-medium" value={formData.latitude} onChange={e=>setFormData({...formData, latitude: e.target.value})} /></div>
                <div><label className="text-sm font-semibold text-slate-700">Kinh độ (Lng)</label><input type="number" step="any" required className="w-full mt-1 p-3 border rounded-xl font-medium" value={formData.longitude} onChange={e=>setFormData({...formData, longitude: e.target.value})} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-sm font-semibold text-orange-600">Cảnh báo (m)</label><input type="number" step="any" required className="w-full mt-1 p-3 border border-orange-200 bg-orange-50 rounded-xl font-bold text-orange-700 outline-none focus:ring-2 focus:ring-orange-500" value={formData.thresholdWarning} onChange={e=>setFormData({...formData, thresholdWarning: e.target.value})} /></div>
                <div><label className="text-sm font-semibold text-red-600">Nguy hiểm (m)</label><input type="number" step="any" required className="w-full mt-1 p-3 border border-red-200 bg-red-50 rounded-xl font-bold text-red-700 outline-none focus:ring-2 focus:ring-red-500" value={formData.thresholdDanger} onChange={e=>setFormData({...formData, thresholdDanger: e.target.value})} /></div>
              </div>
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition shadow-md shadow-blue-500/20">Lưu lại</button>
              </div>
            </form>
          </div>
        </div>
